...
public final class ProceduralTexture530Decoder {
    public Decoded decode(int textureId,byte[] data,int size){
        ...
        for(int i=0;i<count;i++){
            in.u8();int type=in.u8();types.add(type);Node node=create(textureId,type);node.cache=in.u8();int codes=in.u8();
    ...
    private static Node create(int id,int type){switch(type){
        ...
        case 7:return new Combine();case 8:return new Curve();case 10:return new ColorGradient();case 30:return new Range();
    ...
    private abstract static class Node{int cache;Node[] children;abstract int childCount();void decode(int id,int code,BinaryInput in){throw new UnsupportedTextureFormatException(id,"operation parameter "+code+" for "+getClass().getSimpleName());}void finish(int id){}abstract int[] rgb(int x,int y,int size);int mono(int x,int y,int size){int[] c=rgb(x,y,size);return(c[0]+c[1]+c[2])/3;}}
