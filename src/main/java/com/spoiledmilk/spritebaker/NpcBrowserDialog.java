...
final class NpcBrowserDialog extends JDialog {
    ...
    private void diagnose(NpcCatalogEntry entry){if(entry==null||working)return;busy("Checking NPC "+entry.id+"…",true);new SwingWorker<NpcCompatibility,Void>(){protected NpcCompatibility doInBackground()throws Exception{return catalog.assess(entry.id);}protected void done(){try{NpcCompatibility result=get();entry.compatibility(result.category.display);list.repaint();detail.setText(result.summary()+" | standing "+result.standingSequenceId+", walking "+result.walkingSequenceId+" | materials "+result.materialIds);}catch(Exception e){detail.setText("Unsupported: "+root(e));}finally{busy("",false);}}}.execute();}
