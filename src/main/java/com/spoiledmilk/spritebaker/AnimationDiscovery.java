...
public final class AnimationDiscovery {
    ...
    public static List<CombatCandidate> combatCandidates(AnimationWorkspace workspace)throws IOException{
        int standing=workspace.bas==null?workspace.npc.standingAnimation:workspace.bas.standingAnimation,walking=workspace.bas==null?workspace.npc.walkingAnimation:workspace.bas.walkingAnimation;List<CombatCandidate> out=new ArrayList<>();Set<Integer> tried=new LinkedHashSet<>();for(int anchor:new int[]{standing,walking})if(anchor>=0)for(int delta=-8;delta<=8;delta++){int id=anchor+delta;if(id<0||id==standing||id==walking||!tried.add(id))continue;try{Sequence530 sequence=workspace.cache.loadSequence(id);if(sequence.frameIds.length<2||sequence.frameIds.length>20)continue;int score=100-Math.abs(delta)*5-Math.abs(sequence.frameIds.length-6)*2;out.add(new CombatCandidate(id,sequence.frameIds.length,sequence.totalCycles(),score,"near known locomotion ID; review before use"));}catch(Exception ignored){}}
