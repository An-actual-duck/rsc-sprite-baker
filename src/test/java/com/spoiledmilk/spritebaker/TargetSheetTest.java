...
class TargetSheetTest {
    ...
    @Test void projectPersistsSequenceFrameCycleTimeAndLocks(@TempDir Path dir)throws Exception{SpriteProject p=new SpriteProject();p.npcId=72;p.sheet.override(2,5,pose(9));p.sheet.cells[2][5].locked=true;Path file=dir.resolve("project.json");p.save(file);SpriteProject loaded=SpriteProject.load(file);assertEquals(9,loaded.sheet.cells[2][5].pose.sequenceId);assertEquals(4,loaded.sheet.cells[2][5].pose.frameIndex);assertEquals(60,loaded.sheet.cells[2][5].pose.timeMillis);assertTrue(loaded.sheet.cells[2][5].locked);}
