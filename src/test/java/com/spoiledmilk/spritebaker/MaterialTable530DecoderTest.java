...
class MaterialTable530DecoderTest {
    @Test void decodesNeutralParallelArrayFixture(){
        ...
        assertEquals(2,out.length);assertTrue(out[0].present);assertTrue(out[0].softwareEligible);assertFalse(out[0].opaque);assertTrue(out[0].lowDetail);assertTrue(out[0].repeat);
        assertEquals(-2,out[0].scrollU);assertEquals(3,out[0].scrollV);assertEquals(0x1234,out[0].averageColor);assertFalse(out[1].present);
    }
