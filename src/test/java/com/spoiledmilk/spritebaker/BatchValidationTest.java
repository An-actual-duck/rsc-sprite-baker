...
class BatchValidationTest {
    @Test void validatesSupportedProjectSchemas(@TempDir Path directory)throws Exception{Path one=directory.resolve("one.json"),two=directory.resolve("two.json"),three=directory.resolve("three.json"),bad=directory.resolve("bad.json");Files.writeString(one,"{\"schemaVersion\":1}");Files.writeString(two,"{\"schemaVersion\":2}");Files.writeString(three,"{\"schemaVersion\":3}");Files.writeString(bad,"{\"schemaVersion\":4}");assertEquals(1,BatchProcessor.validateProjectSchema(one));assertEquals(2,BatchProcessor.validateProjectSchema(two));assertEquals(3,BatchProcessor.validateProjectSchema(three));assertThrows(java.io.IOException.class,()->BatchProcessor.validateProjectSchema(bad));}
