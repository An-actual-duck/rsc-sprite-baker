...
3. Select a timeline frame or scrub its exact client-time position.
4. Replace the selected cell, or apply the pose to a shared movement row.

The source status displays sequence ID, frame index, within-frame cycle,
milliseconds, direction, and all three assigned role IDs. Timeline thumbnails
and the rendered source preview use the selected cell's direction by default;
the nearby Direction selector makes a different view explicit.

Play source samples the browsed sequence against its encoded frame durations
on a 20 ms timer. Each displayed pose is generated by the normal model poser
and styled renderer, including project/sequence tweening. Play assembled uses
the actual assigned sheet cells in the selected direction. Movement columns
loop Standing, Left step, Standing, Right step; Combat side loops its three
independently assigned combat poses. Each assembled pose remains visible for
the encoded duration of its selected source frame. Both controls support
pause/resume and stop on NPC changes, editor closure, or invalidated project
state.
